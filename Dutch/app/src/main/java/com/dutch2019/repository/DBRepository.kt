...
class DBRepository(private val recentDB: RecentLocationDB) {
    suspend fun getRecentData(): List<LocationSearchData> =
        recentDB.locationDataDao().getAll().reversed()

    suspend fun insertRecentData(data: LocationSearchData) = recentDB.locationDataDao().insert(data)

    suspend fun deleteRecentData(data: List<LocationSearchData>){
        data.forEach{
